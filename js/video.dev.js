/*
修改作者：爱星雾月 wti0.com
删除 innerHTML: '<div class="vjs-control-content"><span class="vjs-control-text">' + (this.buttonText || 'Need Text') + '</span></div>',
全屏对应删除this.el_.children[0].children[0].innerHTML = 'Non-Fullscreen';  与后面一句
添加全屏样色标记vjs.addClass(this.el_, 'vjs-newmax');
label: function(){ return options['kind'] + ' off'; }, 改 -》label: function(){ return ' 关闭字幕'; },
添加 这vjs.removeClass(this.el_, 'vjs-newpaused'); 与vjs-newplaying样色 用于播放暂停图片
删除vjs.FullscreenToggle.prototype.buttonText = 'Fullscreen';

添加按钮样色开始播放vjs-newplaying 暂停样色vjs-newplaying 

修正播放时的正在播放与暂停的图片翻转状态2014年12月3日14:49:40
...
var vjs = function(id, options, ready){
  ...
  // 检查标签
  if (!tag || !tag.nodeName) {
    throw new TypeError('ID无效'); 
  }

///元件可以有一个播放器属性指的是一个已经创建的播放器实例。
//如果不是，建立一个新的播放器和返回。
...
vjs.PlayToggle.prototype.onPlay = function(){
  vjs.removeClass(this.el_, 'vjs-paused');
  vjs.removeClass(this.el_, 'vjs-newpaused');
  vjs.addClass(this.el_, 'vjs-playing vjs-newplaying');
...
vjs.PlayToggle.prototype.onPause = function(){
  vjs.removeClass(this.el_, 'vjs-playing');
  vjs.removeClass(this.el_, 'vjs-newplaying');
  vjs.addClass(this.el_, 'vjs-paused vjs-newpaused');
...
vjs.FullscreenToggle.prototype.onClick = function(){
  if (!this.player_.isFullScreen) {
    ...
    vjs.addClass(this.el_, 'vjs-newmax');
  } else {
    this.player_.cancelFullScreen();
    //this.el_.children[0].children[0].innerHTML = 'Fullscreen'; 
     vjs.removeClass(this.el_, 'vjs-newmax');
  }
};
/**
